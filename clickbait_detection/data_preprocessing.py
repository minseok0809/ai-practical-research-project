...
def write_jsontext_to_xlsx_file_with_batch_size(source_file_list, xlsx_file_path_list, batch_size):
    ...
    for i in range(len(source_file_list)):
        ...
        title, content = make_source(one_json_sample)
        # source = preprocess_source(source)
       
        if len(title_list) >= batch_size:
            num += 1
            if 'Non' in source_file:
                label_list = [0] * len(title_list)

            elif 'Non' not in source_file:
                label_list = [1] * len(title_list)
            
            source_df = pd.DataFrame({'Text':title_list, 'Content':content_list, 'Label':label_list})
...
def write_xlsxtext_to_list_merge_file(xlsx_folder, dataset_folder):
    ...
    for load_path in train_xlsx_path:
        ...
        contents = train_xlsx['Content']
        labels = train_xlsx['Label']
        for text, content, label in zip(texts, contents, labels):
            text_label = [text, content, label]
